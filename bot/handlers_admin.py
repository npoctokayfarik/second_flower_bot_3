...
from .keyboards import kb_open_bot_for_buy, kb_admin_payout, kb_seller_send_card
...
def make_sold_caption(caption: str) -> str:
    cleaned = remove_contacts_from_caption(caption)
    if "ПРОДАНО" in cleaned:
        return cleaned
    return cleaned + "\n\n✅ <b>ПРОДАНО</b>"
...
async def set_admin_card(m: Message, admin_ids: set[int]):
    if not is_admin(m.from_user.id, admin_ids):
        return
    await m.answer(f"Карта сейчас берётся из ENV:\n\n<code>{cfg.admin_card}</code>")
...
async def catch_examples(m: Message, db: DB, admin_ids: set[int]):
    ...
    await m.answer("✅ Примеры сохранены.")
...
async def admin_publish(cb: CallbackQuery, db: DB, admin_ids: set[int], channel_id: int):
    ...
    if len(media_items) == 1:
        ...
        return await cb.message.answer("✅ Опубликовано")
    ...
    try:
        ...
        if group:
            await cb.bot.send_media_group(channel_id, media=group)
...
async def admin_reject(cb: CallbackQuery, db: DB, admin_ids: set[int]):
    ...
    try:
        await cb.bot.send_message(
            listing.user_id,
            f"❌ Заявка отклонена (ID {listing_id}).\nСтоимость размещения: {fmt_sum(cfg.ad_price)} сум."
        )
    except Exception:
        pass
...
async def deal_paid_confirm(cb: CallbackQuery, db: DB, admin_ids: set[int]):
    ...
    try:
        await cb.bot.send_message(deal.buyer_id, f"✅ Администратор подтвердил оплату по сделке #{deal_id}.")
    except Exception:
        pass
...
async def deal_problem(cb: CallbackQuery, db: DB, admin_ids: set[int]):
    ...
    for admin_id in admin_ids:
        try:
            await cb.bot.send_message(admin_id, f"⚠️ Проблема по сделке #{deal_id}. Нужна ручная проверка.")
        except Exception:
            pass

    await cb.message.answer("Администратор получил уведомление ⚠️")
...
async def deal_payout_done(cb: CallbackQuery, db: DB, admin_ids: set[int], channel_id: int):
    ...
    sold_caption = make_sold_caption(listing.public_caption)
    await db.set_listing_sold(listing.id, sold_caption)
