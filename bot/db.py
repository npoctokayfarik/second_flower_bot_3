...
class DB:
    ...
    async def _ensure_columns(self, db: aiosqlite.Connection) -> None:
        ...
        listing_cols = {row[1] for row in await cur.fetchall()}
        if "channel_first_message_id" not in listing_cols:
            await db.execute("ALTER TABLE listings ADD COLUMN channel_first_message_id INTEGER")
        if "channel_control_message_id" not in listing_cols:
    ...
    async def set_listing_reserved(self, listing_id: int, caption: str) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "UPDATE listings SET status='reserved', public_caption=? WHERE id=?",
                (caption, listing_id),
            )
            await db.commit()

    async def set_listing_sold(self, listing_id: int, caption: str) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "UPDATE listings SET status='sold', public_caption=? WHERE id=?",
                (caption, listing_id),
            )
            await db.commit()
        ...
        async with aiosqlite.connect(self.path) as db:
            cur = await db.execute("""
              INSERT INTO deals (
                ...
                price, commission_amount, seller_payout_amount, status
              )
    ...
    async def set_deal_payment_file(self, deal_id: int, file_id: str) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "UPDATE deals SET buyer_payment_file_id=?, status='buyer_paid_pending_admin' WHERE id=?",
                (file_id, deal_id),
            )
            await db.commit()
    ...
    async def confirm_buyer_paid(self, deal_id: int) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute("UPDATE deals SET status='buyer_paid' WHERE id=?", (deal_id,))
            await db.commit()

    async def reject_buyer_paid(self, deal_id: int) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "UPDATE deals SET status='waiting_buyer_payment', buyer_payment_file_id='' WHERE id=?",
                (deal_id,),
            )
            await db.commit()
    ...
    async def set_seller_card(self, deal_id: int, card: str) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "UPDATE deals SET seller_card=?, status='waiting_seller_delivery' WHERE id=?",
                (card, deal_id),
            )
            await db.commit()
    ...
    async def set_seller_delivery_file(self, deal_id: int, file_id: str) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "UPDATE deals SET seller_delivery_file_id=?, status='waiting_buyer_confirmation' WHERE id=?",
                (file_id, deal_id),
            )
            await db.commit()
    ...
    async def set_buyer_confirmed(self, deal_id: int) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute("UPDATE deals SET status='buyer_confirmed' WHERE id=?", (deal_id,))
            await db.commit()

    async def set_problem(self, deal_id: int) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute("UPDATE deals SET status='problem' WHERE id=?", (deal_id,))
            await db.commit()

    async def set_payout_done(self, deal_id: int) -> None:
        async with aiosqlite.connect(self.path) as db:
            await db.execute("UPDATE deals SET status='seller_paid' WHERE id=?", (deal_id,))
            await db.commit()